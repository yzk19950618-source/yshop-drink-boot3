...
public interface StoreOrderMapper extends BaseMapperX<StoreOrderDO> {
    ...
    default PageResult<StoreOrderDO> selectPage(StoreOrderPageReqVO reqVO) {
        ...
        // 后台删除订单仅标记 is_system_del=1；列表默认应隐藏，仅在「已删除」tab（STATUS_6）中展示
        boolean showDeletedOnly = false;
        if (reqVO.getOrderStatus() != null) {
            AdminOrderStatusEnum statusEnum = AdminOrderStatusEnum.toType(reqVO.getOrderStatus());
            showDeletedOnly = (statusEnum == AdminOrderStatusEnum.STATUS_6);
        }
        if (!showDeletedOnly) {

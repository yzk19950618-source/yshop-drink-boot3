...
public class AppStoreOrderServiceImpl extends ServiceImpl<StoreOrderMapper,StoreOrderDO> implements AppStoreOrderService {
    ...
    @Resource
    private ObjectProvider<WebSocketSenderApi> webSocketSenderApi;

    @Value("${yshop.pay.weixin-mock-enabled:false}")
    private boolean weixinMockEnabled;

    private static final String LOCK_KEY = "cart:check:stock:lock";
    private static final String STOCK_LOCK_KEY = "cart:do:stock:lock";

    private static final String WS_MSG_STORE_ORDER_NOTIFY = "store-order-notify";

    /**
     * 向管理端广播门店订单事件（创建 / 已支付），供后台 WebSocket 实时提醒。
     */
    private void notifyAdminStoreOrderNotify(String event, String orderId, String shopName,
                                             BigDecimal payPrice, String orderType, boolean paid) {
        WebSocketSenderApi ws = webSocketSenderApi.getIfAvailable();
        if (ws == null || StrUtil.isBlank(orderId)) {
            return;
        }
        Map<String, Object> payload = new LinkedHashMap<>();
        payload.put("event", event);
        payload.put("orderId", orderId);
        payload.put("shopName", shopName != null ? shopName : "");
        payload.put("payPrice", payPrice != null ? payPrice : BigDecimal.ZERO);
        payload.put("orderType", orderType != null ? orderType : "");
        payload.put("paid", paid);
        ws.sendObject(UserTypeEnum.ADMIN.getValue(), WS_MSG_STORE_ORDER_NOTIFY, payload);
    }
    ...
    public Map<String, Object> createOrder(Long uid,  AppOrderParam param) {
        ...
        if (lock.tryLock()){
            try {
                for (int i = 0;i < productIds.size();i++){
                    ...
                    Integer cartNum = Integer.valueOf(numbers.get(i));
                    StoreProductDO storeProductDO = appStoreProductService.getById(productId);
                    if (storeProductDO == null) {
                        throw exception(STORE_PRODUCT_NOT_EXISTS);
                    }
                    int skuStock = appStoreProductService.getProductStock(productId, newSku, "");
                    int productStock = storeProductDO.getStock() == null ? 0 : storeProductDO.getStock();
                    int availableStock = Math.max(skuStock, productStock);
                    log.info("[createOrder][stock-check] productId={}, sku={}, skuStock={}, productStock={}, buyNum={}",
                            productId, newSku, skuStock, productStock, cartNum);
                    if (availableStock < cartNum) {
                        throw exception(new ErrorCode(1008003010, storeProductDO.getStoreName() + "库存不足" + cartNum));
                    }
                    totalNum += cartNum;
                    ...
                    BigDecimal unitPrice = storeProductAttrValue == null ? storeProductDO.getPrice() : storeProductAttrValue.getPrice();
                    if (unitPrice == null) {
                        unitPrice = BigDecimal.ZERO;
                    }
                    sumPrice = NumberUtil.add(sumPrice, NumberUtil.mul(cartNum, unitPrice));
                }
        ...
        if (StrUtil.isNotBlank(storeOrder.getOrderId())) {
            notifyAdminStoreOrderNotify("created", storeOrder.getOrderId(), storeOrder.getShopName(),
                    storeOrder.getPayPrice(), storeOrder.getOrderType(), false);
        }
    ...
    public Map<String, Object> pay(Long uid, AppPayParam param) {
        ...
        PayTypeEnum payChannel = PayTypeEnum.toType(param.getPaytype());
        if (payChannel == null) {
...
//                            msg, price, param.getUni());
                ...
                break;
            case WEIXIN_MOCK:
                if (!weixinMockEnabled) {
                    throw exception(PAY_WEIXIN_MOCK_DISABLED);
                }
                if (orderInfo == null) {
                    throw exception(STORE_ORDER_NOT_EXISTS);
                }
                this.paySuccess(orderInfo.getOrderId(), PayTypeEnum.WEIXIN.getValue());
                map.put("status", "ok");
                map.put("trade_type", "MOCK_JSAPI");
                break;
            case YUE:
...
//                }
    ...
    public void paySuccess(String orderId, String payType) {
        ...
        notifyAdminStoreOrderNotify("paid", orderInfo.getOrderId(), orderInfo.getShopName(),
                orderInfo.getPayPrice(), orderInfo.getOrderType(), true);
...
//        }
    ...
    private BigDecimal getGainIntegral(List<String> productIds) {
        ...
        for (int i = 0;i < productIds.size();i++){
            ...
            if (storeProductDO == null || storeProductDO.getGiveIntegral() == null
                    || storeProductDO.getGiveIntegral().compareTo(BigDecimal.ZERO) <= 0) {
                continue;
            }
